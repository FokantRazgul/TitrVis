...
| 19 | Impact waves | VERIFIED | `SurfaceSimulation.addImpact`, forced shallow-water slosh (`equilibriumSlope`), `Liquid.tsx` surface + rim-following wall | U `impacts create a depression that propagates and decays`; U `surfaceSlosh.test.ts` (static tilt followed, 2.5 Hz drive → rotating crest within the cap, slope vs renderer quaternion, √(g·depth), rim continuity); B swirl screenshots (rim ±7 mm, decay after release) |
...
| 23 | Flask tilts and rotates while stirring | VERIFIED | `Scene.tsx::FlaskAssembly` (tilt 12°, 2.5 Hz orbit); liquid surface stays level in the world and sloshes relative to the glass | E tiltRad > 0.15 while held; U level-surface slope vs the renderer's quaternion |
...
| `npm test` (Vitest, jsdom) | 102 / 102 passed — chemistry 53 (incl. the 9 required tests, robustness, property test over 300 random configurations, spectra, colour, databases), store 11, CPU simulation 23 (drops, surface waves, 5 sloshing tests: static tilt followed, resonant rotating crest within the cap, level-surface slope vs the renderer's quaternion, √(g·depth) wave speed, rim continuity of the extended field; 8 vortex-ring tests: divergence-free field, floor condition, stagnation speeds, deceleration, lifecycle, dilution), local colour LUT 2, UI 12, benchmark 2 |
